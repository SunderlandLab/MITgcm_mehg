...
listJ={'jT','jS','jTs','jSs','jIs','jHa','jHg','jHm'};
...
fprintf('-> in-situ profiles test started\n');
%
...
fprintf('-> in-situ profiles test completed\n\n');
elseif ismember(0,listTests);
fprintf('-> in-situ profiles test omitted (costfunction0011 not found in %s)\n\n',dirRun);
end;
...
if (test1|test2)&myinfo.interactive;
...
if ~isempty(myinfo.dirMds);
  myinfo.ioSize=[90 1170 50];
  %
  if ~isempty(dir(fullfile(dirRun,myinfo.dirMds,'state_2d_set1*')));
  ...
  %
  if ~isempty(dir(fullfile(dirRun,myinfo.dirMds,'state_3d_set1*')));
  ...
  %
  if ~isempty(dir(fullfile(dirRun,myinfo.dirMds,'trsp_3d_set1*')));
  ...
  %
  fil='';
  if ~isempty(dir(fullfile(dirRun,myinfo.dirMds,'trsp_3d_set2*')));
